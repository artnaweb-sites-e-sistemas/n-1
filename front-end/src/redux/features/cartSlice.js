  ...
  reducers: {
    add_cart_product: (state, { payload }) => {
      ...
      if (!isExist) {
        ...
        notifySuccess(`${payload.title} adicionado ao carrinho`);
      } else {
        state.cart_products.map((item) => {
          if (item._id === payload._id) {
            if (item.quantity >= item.orderQuantity + state.orderQuantity) {
              ...
              notifySuccess(`${state.orderQuantity} ${item.title} adicionado(s) ao carrinho`);
            } else {
              notifyError("Não há mais quantidade disponível para este produto!");
              state.orderQuantity = 1;
            }
          }
          return { ...item };
